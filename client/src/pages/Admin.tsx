...
export const Admin = () => {
  ...
  const [selectedSubscription, setSelectedSubscription] =
    useState<string>("Alpaca Basic");
  const subscriptions = ["Alpaca Basic", "Alpaca Insights", "Alpaca Elite"];
  ...
  const handleAddPageClick = async () => {
    try {
      ...
      const updatedPagesResponse = await fetch(
        "http://localhost:3000/content/pages"
      );
      ...
      <div className="adminContentPages">
        {contentPages.map((page, index) => (
          <div key={index} className="contentPage">
            ...
            {/* Visa prenumerationen baserat på krävd nivå */}
